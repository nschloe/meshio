...
def read_ascii_buffer(f):
    ...
    while True:
        ...
        if items[0] == "MeshVersionFormatted":
            ...
            f.readline()
        elif items[0] in ["RequiredVertices", "TangentAtVertices", "Tangents",
                          "Ridges"]:
            msg = ("Meshio doesn't know keyword {}. Skipping.").format(items[0])
            logging.warning(msg)
            num_to_pass = int(f.readline())
            for i in range(num_to_pass): f.readline()
        else:
            if items[0] != "End":
