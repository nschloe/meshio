...
def _translate_zone(points, corner, meshio_type):
    ...
    p = points[zone]
    if np.dot(p[3]-p[0], np.cross(p[1]-p[0], p[2]-p[0])) < 0.:
...
def _write_cell_data(f, cells, cell_data, field_data):
    ...
    if cell_data and all([ "flac3d:zone" in v.keys() for v in cell_data.values() ]):
        if field_data:
            group_names.update(
                { v[0]: k for k, v in field_data.items() if v[1] == 3 }
            )
    else:
...
def _translate_zgroups(cells, cell_data):
    ...
    if cell_data and all([ "flac3d:zone" in v.keys() for v in cell_data.values() ]):
        ...
        for k, v in cells.items(): 
            mapper.update({
                i: z for i, z in zip(np.arange(imap, imap+len(v))+1, cell_data[k]["flac3d:zone"])
            })
            imap += len(v)

        zgroups = {
            k: [] for k in sorted(set([ v for v in mapper.values() ]))
        }
